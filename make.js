...
desc("Bake the current Omarchy theme into the default Vimium UI css");
task("omarchy-theme", [], async () => {
  // Omarchy renders ~/.config/omarchy/themed/vimium.css.tpl into this file on
  // every theme change, so it is always the active palette.
  const home = Deno.env.get("HOME");
  const source = `${home}/.local/state/omarchy/current/theme/vimium.css`;

  let css;
  try {
    css = (await Deno.readTextFile(source)).trimEnd();
  } catch {
    abort(
      `No rendered theme css at ${source}. Install the template with\n` +
        "  cp build_scripts/omarchy/vimium.css.tpl ~/.config/omarchy/themed/\n" +
        "then run `omarchy theme refresh`.",
    );
  }

  const settingsPath = "./lib/settings.js";
  const settings = await Deno.readTextFile(settingsPath);
  const begin = "  // BEGIN omarchy theme css.";
  const end = "  // END omarchy theme css\n";
  const from = settings.indexOf(begin);
  const to = settings.indexOf(end);
  if (from < 0 || to < 0) abort(`Could not find the omarchy css markers in ${settingsPath}`);

  // The css lands inside a template literal, so anything the template engine
  // treats specially has to be escaped.
  const escaped = css.replaceAll("\\", "\\\\").replaceAll("`", "\\`").replaceAll("${", "\\${");
  const block = begin +
    " Generated by `deno run -A make.js omarchy-theme`\n" +
  ...
  await Deno.writeTextFile(settingsPath, settings.slice(0, from) + block + settings.slice(to));
});
